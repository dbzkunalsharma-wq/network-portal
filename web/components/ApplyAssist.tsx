...
export function ApplyAssist({ job }: { job: Job }) {
  ...
  return (
    <div className="mt-5 rounded-2xl border border-line bg-ink/[0.04] p-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <SparkleIcon className={clsx("h-4 w-4", meta.text)} />
          <h3 className="text-sm font-semibold text-ink">Apply assist</h3>
        </div>
        <button
          type="button"
          onClick={() => setEditing((v) => !v)}
          className="text-xs font-medium text-ink/55 underline-offset-2 hover:text-ink hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/40 focus-visible:rounded"
        >
      ...
      {editing && (
        <div className="mt-3 grid grid-cols-1 gap-2 rounded-xl border border-ink/10 bg-ink/[0.03] p-3 sm:grid-cols-3">
          ...
          />
          <p className="text-[11px] text-ink/40 sm:col-span-3">
      ...
      {contact && (
        <div className="mt-3">
          <span className="text-[11px] font-medium uppercase tracking-wide text-ink/40">{contactKind}</span>
          <div className="mt-1 flex items-center gap-2">
            <ContactIcon className="h-4 w-4 shrink-0 text-ink/45" />
            {cHref ? (
              <a
                href={cHref}
                target={cHref.startsWith("http") ? "_blank" : undefined}
                rel="noopener noreferrer"
                className={clsx("break-all text-sm font-medium underline decoration-dotted underline-offset-2 transition-colors hover:text-ink", meta.text)}
              >
                {contact}
              </a>
            ) : (
              <span className="break-all text-sm font-medium text-ink/80">{contact}</span>
            )}
          </div>
        </div>
      )}
        ...
        <button
          ...
          className={clsx(
            ...
            copied
              ...
              : "border-line bg-ink/[0.05] text-ink/75 hover:border-ink/30 hover:bg-ink/[0.1] hover:text-ink"
          )}
        >
  ...
  return (
    <label className={clsx("flex flex-col gap-1", className)}>
      ...
      <input
        ...
        className="rounded-lg border border-line bg-ink/[0.05] px-2.5 py-1.5 text-sm text-ink placeholder:text-ink/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/40"
      />
    </label>
  );
}
